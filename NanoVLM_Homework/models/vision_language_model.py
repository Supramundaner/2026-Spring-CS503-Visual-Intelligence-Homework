...
class VisionLanguageModel(nn.Module):
    ...
    def forward(self, input_ids, image, attention_mask=None, targets=None):
        ...
        combined_embeds = torch.cat([image_embeds, text_embeds], dim=-2)
        ...
        if attention_mask is not None:
            ...
            image_token_length = image_embeds.shape[1]
        ...
        if targets is not None:
            ...
            logits = logits[:, image_token_length:, :] 
    ...
    def generate(self, input_ids, image, attention_mask=None, max_new_tokens=5):
        ...
        image_embd_encoder = self.vision_encoder
        image_embd = image_embd_encoder(image)
        ...
        # Step 3: Concatenate image and text embeddings
        combined_embed = torch.concat([image_embd,token_embd],dim=-2)
        ...
        for i in range(max_new_tokens):
            ...
            outputs = torch.concat([outputs, generated_embed],dim = -2) # concat generated_embed to outputs along dim=1

            if attention_mask is not None:
                attention_mask = torch.concat([attention_mask, torch.ones(batch_size, 1, device = attention_mask.device)],dim = -1)
    ...
    def generate_with_kv_cache(self, input_ids, image, attention_mask=None, max_new_tokens=5):
        ...
        model_out, past_key_values = self.decoder.forward_kv(combined_embed,attention_mask = attention_mask,past_key_values = None)
        ...
        generated_tokens = torch.zeros((batch_size, max_new_tokens), device=input_ids.device, dtype=input_ids.dtype)
        generated_tokens[:,0] = next_token # fill in the generated next token in generated_tokens at 0th position

        # Step 5: DECODE LOOP
        for i in range(1, max_new_tokens):

            next_embed =  self.decoder.token_embedding(next_token)
            attention_mask = torch.concat([attention_mask, torch.ones(batch_size, 1,dtype = attention_mask.dtype, device = attention_mask.device)],dim = -1)
            model_out, past_key_values = self.decoder.forward_kv(next_embed,attention_mask = attention_mask,past_key_values = past_key_values)
            ...
            if (next_token == 2).all(): # stop the generation loop if the generated token == 2 (token id 2 is the EOS token we used during training)
                break
