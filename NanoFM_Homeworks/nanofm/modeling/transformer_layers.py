...
class Attention(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, qkv_bias: bool = False, proj_bias: bool = False):
        ...
        self.qkv_proj = nn.Linear(dim, dim * 3, bias=qkv_bias) # Combined linear layer for Q, K, V projections
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        q, k, v = rearrange(self.qkv_proj(x), "b l (qkv h d) -> qkv b h l d", qkv=3, h=self.num_heads) 
...
class CrossAttention(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, qkv_bias: bool = False, proj_bias: bool = False):
        ...
        self.q_proj = nn.Linear(dim, dim, bias=qkv_bias) # Linear layer for Q projection
        ...
        self.kv_proj = nn.Linear(dim, dim * 2, bias=qkv_bias) # Combined linear layer for K and V projections
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        k, v = rearrange(self.kv_proj(context), "b m (kv h d) -> kv b h m d", kv=2, h=self.num_heads)
...
class Block(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, mlp_ratio: float = 4., use_bias: bool = False):
        ...
        self.attn = Attention(dim, head_dim, qkv_bias=use_bias, proj_bias=use_bias) # TODO Attention layer
